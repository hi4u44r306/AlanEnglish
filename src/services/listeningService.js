...
const callListeningFunction = async (functionName, firebaseUser, body) => {
    ...
    if (!response.ok) {
        throw new Error(result?.error || `${functionName} 執行失敗`);
    }
...
export const getBookPlaybackProgress = async (firebaseUser, bookId) => {
    return await callListeningFunction("get-playback-progress", firebaseUser, {
        book_id: bookId
    });
};

export const recordTrackPlay = async (firebaseUser, trackId) => {
    return await callListeningFunction("record-play", firebaseUser, {
        track_id: trackId
    });
};