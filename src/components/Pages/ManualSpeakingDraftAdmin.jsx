...
const validateDraft = (form, rows) => {
    ...
    const rowErrors = rows.map(row => {
        ...
        if (row.interaction_type === "standard_sentence") {
            ...
            else if (!textQaPromptIsComplete(row.prompt_text)) errors.prompt_text = "需包含完整英文問句；中文提示可放句首、句中或句尾";
...
export default function ManualSpeakingDraftAdmin({ firebaseUser, books, onCreated }) {
    ...
    const [pdfCropTargetKey, setPdfCropTargetKey] = useState("");
        ...
        <form className="platform-form" onSubmit={submit} noValidate>
            ...
            <div className="speaking-picture-authoring__rows">{rows.map((row, index) => {
                ...
                return <article key={row.key} className={validationRequested && Object.keys(errors).length ? "speaking-draft-row--invalid" : ""}><header><strong>第 {index + 1} 題</strong><button type="button" className="platform-danger" disabled={working || rows.length <= 1} onClick={() => removeRow(row.key)}><Trash2 size={16} />刪除</button></header><div className="platform-form">
                    ...
                    {row.interaction_type === "text_qa" && <><label className={invalid("prompt_text") ? "speaking-draft-field--invalid" : ""}><span>學生看到的完整問句（可加入中文提示）</span><input value={row.prompt_text} onChange={event => updateRow(row.key, "prompt_text", event.target.value)} placeholder="What are those?（尺）" disabled={working} aria-invalid={Boolean(invalid("prompt_text"))} /><FieldError id={id("prompt_text")} message={errors.prompt_text} visible={validationRequested} /></label><label className={invalid("answer_text") ? "speaking-draft-field--invalid" : ""}><span>完整示範回答（學生作答前不顯示）</span><input value={row.answer_text} onChange={event => updateRow(row.key, "answer_text", event.target.value)} placeholder="Seven minus two is five." disabled={working} aria-invalid={Boolean(invalid("answer_text"))} /><FieldError id={id("answer_text")} message={errors.answer_text} visible={validationRequested} /></label></>}
