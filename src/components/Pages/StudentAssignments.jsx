                ...
                {loading ? (
                    ...
                    <article className={`assignment-card student-assignment-card ${item.progress?.completed ? "completed" : ""}`} key={item.id}>
                        ...
                        {item.source_type === "ai_material" ? (
                            ...
                            <>
                                <div className="student-assignment-score">
                                    <div><span>已播放</span><strong>{item.progress?.play_count || 0} / 7</strong></div>
                                    <div><span>完成條件</span><strong>7 次</strong></div>
                                </div>
                                <Link className="assignment-primary link" to={`/student/books/${item.track?.book?.code || ""}`}>前往聽力教材</Link>
                            </>
                        )}
                    </article>
                ))}
            ...
            {activeAssignment?.material && (
                <section className="assignment-card assignment-quiz" id="assignment-quiz">
                    ...
                    {result ? (
                        ...
                        <button type="button" className="assignment-primary" onClick={handleSubmit} disabled={submitting}>{submitting ? "批改中..." : "提交答案"}</button>
                    )}
                </section>
            )}
        </main>
