...
export default function StoreHeader({ salesPaused = false }) {
    ...
    return <header className="commerce-site-header store-header">
        ...
        <nav aria-label="教材商城導覽">
            ...
            {!salesPaused && <Link to="/shop/cart"><FiShoppingCart />購物車{cartCount > 0 && <b>{cartCount}</b>}</Link>}
            {!authLoading && (user
                ? <><Link to="/shop/orders"><FiPackage />{salesPaused ? "查詢既有訂單" : "我的訂單"}</Link><button type="button" onClick={signOut}><FiLogOut />商城登出</button></>
                : <Link to={salesPaused ? "/shop/login?next=/shop/orders" : "/shop/login"}><FiLogIn />{salesPaused ? "查詢既有訂單" : "商城登入"}</Link>)}
        </nav>
    </header>;
}
