...
const pageLabelFor = questionSet => {
    const sourcePage = questionSet?.generation_metadata?.source_pages?.[0];
    return sourcePage ? `P${sourcePage}` : "";
};
...
const emptyQuestion = () => ({
    ...
    pronunciation_notes_zh: "", alt_zh: "", file: null
});
...
export default function SpeakingPictureQuestionSetEditor({ firebaseUser, questionSet, onChanged }) {
    ...
    const expectedCount = EXPECTED_COUNTS[templateKey] || questions.length;
    ...
    const saveQuestion = async () => {
        ...
        try {
            ...
            if (form.file) {
                await uploadSpeakingQuestionPicture(firebaseUser, Number(questionId), pageLabel, form.alt_zh, form.file);
            }
    ...
    const remove = async () => {
        if (!selectedQuestion || !window.confirm(`確定刪除第 ${selectedIndex + 1} 題嗎？草稿發布前仍須補回固定題數。`)) return;
        setWorking("delete");
        try {
    ...
    const startAdd = () => {
        if (questions.length >= expectedCount) return toast.info(`本頁固定 ${expectedCount} 題；請先刪除要替換的題目`);
        setAdding(true);
        setSelectedId(null);
    ...
    return <div className="speaking-picture-editor">
        ...
        <div className="speaking-picture-editor__workspace">
            <aside className="speaking-picture-editor__navigator" aria-label="題目清單">
                <header><strong>{pageLabel} 題目</strong><span>{questions.length}/{expectedCount}</span></header>
                <div>{questions.map((question, index) => <button type="button" key={question.id} className={!adding && Number(question.id) === Number(selectedQuestion?.id) ? "active" : ""} onClick={() => { setAdding(false); setSelectedId(question.id); setForm(readQuestion(question)); setPreview(null); }}>
                    <strong>{index + 1}</strong><span>{asOne(question.speaking_question_interactions)?.prompt_text || question.question_text}</span>
                </button>)}</div>
                <button type="button" className="speaking-picture-editor__add" disabled={questions.length >= expectedCount} onClick={startAdd}><Plus size={17} />新增一題</button>
            </aside>
        ...
        {isGap && <button type="button" className="platform-secondary speaking-picture-editor__audio" disabled={working === "audio" || questions.length !== expectedCount} onClick={prepareAudio}><Volume2 size={17} />{working === "audio" ? "準備女聲中…" : "更新逐字與整句女聲"}</button>}
        {questions.length !== expectedCount && <p className="speaking-picture-editor__warning">發布前必須補齊 {expectedCount} 題；目前有 {questions.length} 題。</p>}
    </div>;
}
