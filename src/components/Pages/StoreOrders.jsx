...
import { loadStoreOrder, loadStoreOrders } from "../../services/storeService";
import StoreHeader from "./StoreHeader";
import "./css/Store.scss";

const PAYMENT = { pending: "等待付款", paid: "付款完成", failed: "付款失敗", expired: "付款逾時", refunded: "已退款" };
const FULFILLMENT = { awaiting_payment: "等待付款", preparing: "準備中", shipping: "運送中", completed: "已完成", cancelled: "已取消" };
const money = value => `NT$${Number(value || 0).toLocaleString("zh-TW")}`;
const StepIcon = ({ status }) => status === "completed" ? <FiCheckCircle /> : status === "shipping" ? <FiTruck /> : status === "preparing" ? <FiPackage /> : <FiClock />;
...
export default function StoreOrders() {
    ...
    return <><StoreHeader /><main className="store-page"><header className="store-page-title"><span>ORDER HISTORY</span><h1>{orderNumber ? `訂單 ${orderNumber}` : "我的歷史訂單"}</h1><p>付款狀態與出貨進度分開顯示；付款完成後才會進入準備中。</p></header>
        {loading ? <section className="store-empty"><p>讀取訂單中…</p></section> : orders.length === 0 ? <section className="store-empty"><FiPackage /><h2>目前沒有訂單</h2><Link to="/shop">前往選購教材</Link></section> : <section className="store-order-list">{orders.map(order => <article key={order.id}>
            ...
            <div className="store-order-progress"><StepIcon status={order.fulfillment_status} /><div><strong>{FULFILLMENT[order.fulfillment_status]}</strong><p>{order.fulfillment_status === "preparing" ? "我們已收到款項，正在整理教材。" : order.fulfillment_status === "shipping" ? "教材已交由物流配送。" : order.fulfillment_status === "completed" ? "這筆訂單已完成。" : "尚未收到 Stripe 的付款確認。"}</p></div></div>
