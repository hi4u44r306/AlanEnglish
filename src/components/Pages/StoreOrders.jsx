...
export default function StoreOrders() {
    ...
    const cancelPayment = async order => {
        if (!window.confirm(`確定取消訂單 ${order.order_number} 的付款嗎？取消後原本的金流付款頁將無法再使用。`)) return;
    ...
    return <><StoreHeader /><main className="store-page"><header className="store-page-title"><span>ORDER HISTORY</span><h1>{orderNumber ? `訂單 ${orderNumber}` : "我的歷史訂單"}</h1><p>付款狀態與出貨進度分開顯示；付款完成後才會進入準備中。</p></header>
        {loading ? <section className="store-empty"><p>讀取訂單中…</p></section> : orders.length === 0 ? <section className="store-empty"><FiPackage /><h2>目前沒有訂單</h2><Link to="/shop">前往選購教材</Link></section> : <section className="store-order-list">{orders.map(order => <article key={order.id}>
            ...
            {order.payment_status === "paid" && (order.learning_access_status === "claimed" ? <section className="store-learning-access is-claimed"><FiCheckCircle /><div><strong>線上學習權限已領取</strong><span>三本教材與 90 天平台權限已帶入通過驗證的學習帳號。</span></div><Link to="/login?next=/student/membership">前往學習平台</Link></section> : <section className="store-learning-access"><FiClock /><div><strong>尚未開通線上學習權限</strong><span>請用商城相同 Email 建立並驗證學習帳號；不需要共用商城密碼。</span></div><Link to="/shop/activate-learning">立即開通</Link></section>)}
