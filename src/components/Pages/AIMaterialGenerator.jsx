...
const TOPIC_IDEAS = {
    reading: ["校園生活", "動物園的一天", "生日派對", "野餐時間", "迷路問路", "我的家人", "天氣與穿著", "去超市購物"],
    vocabulary: ["動物", "食物與飲料", "顏色與形狀", "身體部位", "教室用品", "家庭成員", "運動", "天氣"],
    grammar: ["be 動詞", "can 與能力", "like 與喜好", "have / has", "現在進行式", "there is / are", "Wh- 問句", "時間與日常作息"],
    listening: ["在學校打招呼", "在餐廳點餐", "生日邀請", "動物園導覽", "放學後的計畫", "天氣預報", "問路對話", "購物對話"]
};
...
function AIMaterialGenerator() {
    ...
    const topicIdeas = TOPIC_IDEAS[materialType] || [];
    ...
    const chooseRandomTopic = () => {
        if (!topicIdeas.length) return;
        chooseTopic(topicIdeas[Math.floor(Math.random() * topicIdeas.length)]);
    };
            ...
            {!canGenerate && (
                <section className="ai-addon-banner" aria-labelledby="ai-addon-banner-title">
                    ...
                    <div className="ai-addon-banner-copy">
                        <span>AI MATERIALS & PRONUNCIATION</span>
                        <h2 id="ai-addon-banner-title">AI 教材與發音練習</h2>
                        <p>{isActiveAcademyStudent
                            ? "英文班在校生可用每月 NT$499 加購，包含 AI 教材生成與發音教練。"
                            : isAcademyStudent
                                ? "離校生先啟用 NT$299 基本會員，再以 NT$499 加購 AI 教材與發音練習，兩項合計每月 NT$798。"
                                : "一般會員先啟用 NT$299 基本會員，再以 NT$499 加購 AI 教材與發音練習，兩項合計每月 NT$798。"}</p>
                    </div>
                    <div className="ai-addon-banner-price"><strong>NT$499</strong><span>／月</span></div>
            ...
            {activeTab === "generator" && !canGenerate ? null : activeTab === "generator" ? (
                <section className="ai-generator-shell">
                    <form className="ai-generator-panel" onSubmit={handleGenerate}>
                        ...
                        {materialType !== "custom" && <div className="ai-topic-picker">
                            <div className="ai-topic-picker-heading">
                                <div><span>需要一點靈感嗎？</span><p>選一個主題就可以開始，也可以自己輸入。</p></div>
                                <button type="button" onClick={chooseRandomTopic}><FiRefreshCw /> 幫我想一個</button>
                            </div>
                            <div className="ai-topic-chips" aria-label="教材主題建議">
                                {topicIdeas.map(item => <button type="button" key={item} className={topic === item ? "active" : ""} onClick={() => chooseTopic(item)}>{item}</button>)}
                            </div>
                            <label className="ai-field ai-field-full"><span>教材主題</span><input type="text" value={topic} onChange={event => setTopic(event.target.value)} placeholder={materialType === "grammar" ? "例如：be 動詞、can、現在進行式（系統會依年級限制）" : "例如：動物、學校、旅行、運動"} maxLength={120} /></label>
                        </div>}
