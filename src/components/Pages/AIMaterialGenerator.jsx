                ...
                <form className="ai-generator-panel" onSubmit={handleGenerate}>
                    ...
                    {materialType !== "custom" && (
                        <label className="ai-field ai-field-full">
                            <span>教材主題</span>
                            <input type="text" value={topic} onChange={event => setTopic(event.target.value)} placeholder={materialType === "grammar" ? "例如：過去式、比較級、介系詞" : "例如：動物、太空、旅行、運動"} maxLength={120} />
                        </label>
                    )}

                    <label className="ai-field ai-field-full">
                        <span>{materialType === "custom" ? "告訴 AI 你想要什麼" : "額外需求（選填）"}</span>
                        <textarea value={customRequest} onChange={event => setCustomRequest(event.target.value)} placeholder={materialType === "custom" ? "例如：我要國小五年級程度，主題是去日本旅行，要有單字、短文和閱讀理解題。" : "例如：希望多練習疑問句，題目不要太難。"} rows={4} maxLength={600} />
                    </label>
                    ...
                    <button type="submit" className="ai-generate-button" disabled={generating || loadingUsage || usage.remaining <= 0}>
                        {generating ? <><FiRefreshCw className="ai-spin" /> AI 正在製作教材...</> : <><FiStar /> 生成 {selectedType?.title || "AI 教材"}</>}
                    </button>
            ...
            {content && (
                <section className="ai-result" id="ai-material-result">
                    <div className="ai-result-header">
                        <div>
                            <span className="ai-eyebrow"><FiStar /> GENERATED FOR YOU</span>
                            <h2>{content.title}</h2>
                                    ...
                                    {content.questions.map((question, index) => (
                                        <details className="ai-question-card" key={index}>
                                            ...
                                            <div className="ai-question-options">
                                                {(question.options || []).map((option, optionIndex) => <div key={optionIndex}>{String.fromCharCode(65 + optionIndex)}. {option}</div>)}
                                            </div>
