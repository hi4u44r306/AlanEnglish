...
function FreeTrialSignup() {
    ...
    const submit = async event => {
        ...
        if (!form.name.trim()) return toast.error("請輸入學生姓名");
        if (form.password.length < 6) return toast.error("密碼至少需要 6 個字元");
        if (form.password !== form.confirmPassword) return toast.error("兩次輸入的密碼不一致");
        setSubmitting(true);
        try {
        ...
        <main className="platform-public">
            <section className="platform-public-card">
                ...
                <form className="platform-form" onSubmit={submit}>
                    ...
                    <label><span>登入 Email</span><input name="email" type="email" value={form.email} onChange={update} autoComplete="email" required /></label>
