...
function MembershipAdmin() {
    ...
    const makeCodes = async event => {
        ...
        try { const result = await generateActivationCodes(firebaseUser, { ...codeForm, plan_id: codeForm.plan_id || null, book_id: codeForm.book_id || null, expires_at: codeForm.expires_at || null }); setGeneratedCodes(result.codes || []); toast.success(`已產生 ${result.codes?.length || 0} 組啟用碼`); await load(); }
