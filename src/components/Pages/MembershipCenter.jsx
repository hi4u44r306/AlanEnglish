    ...
    return (
        <main className="platform-page">
            ...
            <section className="platform-card membership-plans" id="plans">
                ...
                {hasAiPremium && <div className="membership-active-addon" role="status" aria-label="AI Premium｜AI 教材與發音練習已啟用"><span className="membership-active-addon-icon"><FiZap aria-hidden="true" /></span><div><span>AI Premium</span><strong>你的 AI 學習力已升級</strong><small>{isActiveAcademyStudent ? "英文班在校期間已包含 · 學籍狀態由老師或管理員管理" : hasAiAddon ? aiAddonCancelling ? `使用至 ${formatDate(aiRenewalAt)}，到期後不再扣款` : aiRenewalDay ? `每月 ${aiRenewalDay} 日續訂 · 每日 5 次、每月 150 次` : "AI 教材與發音練習已啟用" : "AI 教材與發音練習已啟用"}</small></div><div className="membership-active-addon-actions"><Link to="/student/ai-generator">{isActiveAcademyStudent ? "AI 練習" : "AI 教材"}</Link><Link to={isActiveAcademyStudent ? "/student/speaking-challenges" : "/student/pronunciation"}>{isActiveAcademyStudent ? "口說練習" : "發音練習"}</Link>{hasAiAddon && !isActiveAcademyStudent && aiAddonSubscription?.stripe_subscription_id && <button type="button" disabled={Boolean(working)} onClick={() => updateRenewal(aiAddonCancelling, aiAddonSubscription.stripe_subscription_id)}>{aiAddonCancelling ? "恢復續訂" : "到期取消"}</button>}</div></div>}
                {isActiveAcademyStudent
