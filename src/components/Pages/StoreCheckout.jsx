...
import { cancelStoreCheckout, createStoreCheckout, loadStoreConfig } from "../../services/storeService";
...
export default function StoreCheckout() {
    ...
    const cancelledOrder = new URLSearchParams(location.search).get("order") || "";
    ...
    useEffect(() => {
        if (!session || !cancelled || !cancelledOrder || cancelState !== "idle") return;
        setCancelState("working");
        cancelStoreCheckout(session, cancelledOrder)
            .then(() => setCancelState("done"))
            .catch(error => { setCancelState("error"); toast.error(error.message || "無法更新取消付款狀態"); });
    }, [session, cancelled, cancelledOrder, cancelState]);
    ...
    if (!authLoading && cart.length === 0 && !cancelled) return <Navigate to="/shop/cart" replace />;
    ...
    const submit = async event => {
        ...
        try {
            ...
            if (!result.url) throw new Error("安全結帳網址建立失敗");
            window.location.assign(result.url);
        } catch (error) { toast.error(error.message || "目前無法結帳"); setCheckoutRequestId(window.crypto.randomUUID()); setBusy(false); }
    };

    return <><StoreHeader /><main className="store-page"><header className="store-page-title"><span>DELIVERY & PAYMENT</span><h1>寄送資料與結帳</h1><p>付款會前往安全金流頁面；只有後端確認收到款項後，訂單才會進入準備中。</p></header>
        {cancelled && <div className="store-notice store-notice-danger">{cancelState === "working" ? "正在確認取消付款…" : cancelState === "error" ? "付款頁已關閉，但訂單狀態更新失敗；請到「我的訂單」再次取消。" : "你已取消付款，這筆訂單不會出貨。"}</div>}
