...
export default function SpeakingContentAdmin() {
    ...
    return <main className="platform-page speaking-content-admin">
        <header className="platform-hero"><div><span className="platform-eyebrow">TEXTBOOK TO SPEAKING</span><h1>教材 AI 口說題庫</h1><p>貼入並核對教材文字，讓 AI 根據教材主題規劃問題、提示與示範回答。所有 AI 內容都先保存為草稿，必須人工審核後才能發布。</p></div></header>

        <section className="platform-card speaking-workflow" aria-label="製作流程">
            <div><FileText /><strong>1. 教材文字</strong><span>貼入並核對頁碼</span></div><div><Sparkles /><strong>2. AI 草稿</strong><span>一次產生共用題庫</span></div><div><CheckCircle2 /><strong>3. 人工發布</strong><span>逐題修改後上線</span></div>
        </section>

        <section className="platform-card">
            <div className="platform-section-title"><div><span className="platform-eyebrow">REVIEWED SOURCE</span><h2>建立核准教材來源</h2></div></div>
            <form className="platform-form" onSubmit={saveSource}>
                ...
                <label><span>已辨識並人工核對的教材文字</span><textarea required minLength="20" rows="12" value={source.source_text} onChange={event => updateSource("source_text", event.target.value)} placeholder="第一版先貼入教材文字。PDF／圖片 OCR 與私人檔案上傳會在下一階段加入。" /></label>
                <label className="speaking-confirm"><input type="checkbox" checked={source.confirmed} onChange={event => updateSource("confirmed", event.target.checked)} /><span>我已確認這段文字、教材、Unit 與頁碼正確，允許 AI 以此為唯一出題來源。</span></label>
                <button className="platform-primary" disabled={working === "source"}>{working === "source" ? "儲存中…" : "儲存核准來源"}</button>
            </form>
        </section>
        ...
        <section className="platform-card">
            ...
            {loading ? <div className="platform-loading">題庫載入中…</div> : sourceRows.length === 0 ? <div className="platform-empty"><BookOpen /><strong>尚未建立教材來源</strong><p>先在上方貼入並核對第一個教材單元。</p></div> : <div className="speaking-source-list">{sourceRows.map(section => <article className="speaking-source-card" key={section.id}>
                <header><div><span>{section.document?.title || "教材來源"}</span><h3>{section.topic}</h3><p>{section.unit_label || "未標示單元"} · {section.page_from_label || "未標示頁碼"}{section.page_to_label ? `–${section.page_to_label}` : ""} · {section.language_level}</p></div><button type="button" className="platform-primary" disabled={working === `generate-${section.id}`} onClick={() => generate(section)}><Sparkles size={17} />{working === `generate-${section.id}` ? "AI 產生中…" : "產生新版草稿"}</button></header>
