...
const DraftReadinessPanel = ({ readiness, interactionType }) => readiness.ready
    ? <div className="speaking-readiness is-ready"><CheckCircle2 /><div><strong>內容檢查完成</strong><span>{interactionType === "picture_gap_sentence" ? "發布時會自動產生或更新停頓整句語音。" : interactionType === "standard_sentence" ? "發布時會自動產生或更新全部示範語音。" : interactionType === "text_qa" ? "可以直接以純文字問答發布，不會產生示範語音。" : "可以預覽學生畫面並發布。"}</span></div></div>
    ...
    return <details className="speaking-student-preview">
        ...
        <div className="speaking-student-preview__screen">
            ...
            <div className="speaking-student-preview__questions">{questions.map((question, index) => <article key={question.id}>
                ...
                {!isPictureSet && interactionType !== "text_qa" && <QuestionAudioPreview firebaseUser={firebaseUser} questionSet={questionSet} question={question} />}
                {!isPictureSet && <small>{question.pronunciation_notes_zh || "完成錄音後顯示發音回饋。"}</small>}
            </article>)}</div>
            <p className="speaking-student-preview__note">{interactionType === "text_qa" ? "這是管理員內容預覽；發布後學生閱讀題目、直接口說回答，可使用錄音回聽與逐字發音回饋，不播放示範音檔。" : "這是管理員內容預覽；發布後學生可使用示範語音、錄音回聽與逐字發音回饋。"}</p>
        </div>
    </details>;
};
...
export default function SpeakingContentAdmin() {
    ...
    const publish = async questionSet => {
        ...
        const interactionType = String(questionSet.generation_metadata?.interaction_type || "");
        const pageLabels = questionSetPageLabels(questionSet, section);
        const publishPreparation = interactionType === "text_qa"
            ? "這是純文字問答，不會產生或播放示範音檔"
            : "系統會先完成必要語音";
        ...
        try {
            ...
            } else if (!["picture_qa", "text_qa", "alphabet_round", "letter_spelling"].includes(interactionType)) {
