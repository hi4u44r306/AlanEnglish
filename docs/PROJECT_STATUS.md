...
- 家長通知信品牌 Logo（測試站後端已部署，正式站尚未部署）：`guardian-email` 的家長週報、個別學習提醒與既有通知草稿寄送三種 HTML 模板，均在深藍標頭加入既有 Alan English 紅色 A 圖形 Logo、固定尺寸及替代文字；圖片沿用正式網站公開 HTTPS 資源，已確認回應 200 與 `image/png`。寄信合約 6／6、全部 Edge Function 語法、Production build 與 `git diff --check` 均成功；測試 `guardian-email` v24 為 ACTIVE，OPTIONS 200、未登入 POST 401。未為避免打擾家長而自動寄出測試信，仍待管理員以單一測試收件人驗收 Gmail 圖片顯示。Gmail 寄件者名稱旁的頭像需另外完成 BIMI、DMARC 及憑證／DNS 設定，本批未修改 DNS。
