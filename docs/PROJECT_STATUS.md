...
本次 Workbook 3 P5／P7／P9 逐頁候選 `invalid_output` 修正（2026-09-23，尚未部署）：
...
- 不需 migration，不修改或覆蓋既有草稿、已發布題庫及學生紀錄。OCR／性別規則 5/5、口說答案判定、管理頁 18/18、Edge Function 語法與 `git diff --check` 已通過；完整口說契約 31/32，唯一失敗仍是既有手機導覽 CSS selector 斷言，與本批逐頁候選無關。Production build、正式部署及 P5／P7／P9 實際重跑待完成。
