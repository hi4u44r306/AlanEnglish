...
- Google TTS 國小生語速與清晰度修正（本機完成，尚未部署）：保留 `en-US-Chirp3-HD-Leda`，將示範語速由原生 `1.0` 降至 `0.82`，輸出由壓縮 MP3 改為未壓縮 LINEAR16 WAV，設定版本更新為 `elementary-clear-v3`。新設定雜湊會建立並連結新版資產，舊 MP3 保留不覆蓋；新增 additive migration `20260904143848_speaking_tts_wav_quality` 僅放寬資產格式檢查為 MP3／WAV。尚待套用測試 migration、部署 `speaking-tts-manager` 與測試站後，補產生少量題目進行真人音質驗收。
