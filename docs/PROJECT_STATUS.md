...
- 管理員 API 成本控制中心（測試站已部署，正式尚未部署）：`/admin/api-usage` 已改為互動式月用量儀表板，整合既有 OpenAI AI 教材紀錄、口說 OCR／題庫生成 Token 與 Google Cloud TTS 字元數，顯示總預算進度、月底預估、每日趨勢、成功率及預算／失敗率／單日突增警示。Supabase、Cloudflare R2、Netlify、Firebase Authentication、Resend 與 PAYUNi 以「外部核對」卡呈現，未知帳單不會誤顯示為零元。未新增 migration；遠端已核對 5 張既有資料表及必要欄位。`generate-ai-material` 已部署為 ACTIVE v28，未登入請求正確回 401；固定測試站 deploy `6a9adedc0210743a91e846fc` 回應 200。尚待已登入管理員驗收真實本月資料與儲存預算設定。
