...
- Workbook 2 第一個口說大關卡（本機完成，尚未推送／部署）：已依 120 頁教師版目錄及 P56～P59 實際頁面建立 `docs/speaking-content/WORKBOOK_2_SPEAKING_MAP.md`，先排除描寫格、無圖片脈絡的 `What is this?` 與未確認授權的歌曲。第一個可直接使用的主題選為 P56、P58「我來自哪裡？」，管理員可一鍵建立六題人工精選草稿，練習個人 `I am from [你的國家]`、he／she／they 及 `come from`；不執行 OCR、不呼叫付費出題 AI、不自動發布，並以 `workbook_2_origin_places_v1` 防止重複建立。此批建立可繼續擴充 Workbook 3～5 的共用精選關卡設定，不新增 migration。管理頁與服務層相關 2 suites／8 tests、題庫契約 12／12、全前端 46 suites／150 tests、Edge Function 語法、production build 與 `git diff --check` 均成功；尚待 checkpoint、推送、部署 `speaking-content-manager` 與測試站。
...
- 口說題庫男女聲交錯示範（需求已確認，尚未實作／部署）：管理員建立關卡、發布題庫或補產生示範語音時，不再全部固定使用同一位女聲；預計建立適合國小生、清楚且明亮的 Google Cloud TTS 男聲／女聲核准池，依同一題庫內的小關卡順序做男女聲平衡交錯，避免純隨機連續抽到同一性別。每題在生成時即保存實際 `voice_id` 與資產設定雜湊，學生重播時沿用同一音檔，不在每次播放時重新隨機或重複計費；題目修正版預設沿用原聲音，管理員可在預覽中手動指定男聲、女聲、保留原聲或要求重新分配。既有已產生的 Autonoe 語音不自動覆寫，待管理員選擇批次補產生才建立新版私人 R2 資產。實作時須同步補上聲音預覽、失敗重試、題庫版本相容、TTS 成本統計、測試與網站使用手冊。
