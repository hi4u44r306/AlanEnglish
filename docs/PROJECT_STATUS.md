...
- 口說手機單一捲動介面（本機已完成，尚待測試站部署）：取消固定 `100svh` 與中央題目卡 `overflow-y: auto`，改為整頁唯一捲動；關卡名稱／進度黏在 Navbar 下方，上一題／下一題固定於底部並保留 safe area。手機版移除最外層完成綠框與陰影，題目改為單一白色資訊面，三階段改為分段控制器，發音提醒收合時固定 44px，降低框中框與雙層滑動的不直覺感。尚待相關測試、production build 與測試站實際驗收。
