...
- Workbook 1 A–Z 學生體驗第二批（本機完成，尚未部署）：手機介紹頁改為每列五張大字 `Aa`～`Zz`，大小寫均清楚置中；完整播放目前主音檔後，新增 server-only 導聽完成紀錄，綁定學生、題組版本與 audio fingerprint，後續可直接開始，換主音檔會自動要求重新導聽。A–Z 回合新增一次 server-side retry：第一次辨識錯誤顯示「系統剛剛聽到」與同題提示，維持麥克風串流並於三秒後自動再試；第二次錯誤才安全歸零。新增 additive migration `20260914000000_speaking_alphabet_intro_progress.sql`，所有新資料表與 RPC 僅授權 service role；本機 `WorkbookOneFoundationChallenge` 21/21、教材口說契約 23/23、Edge syntax 成功。尚待 production build、commit／push，以及專案擁有者明確同意後才可套用 migration、部署 `speaking-challenge` 與 `pronunciation-coach` 至共用 Supabase，再發布固定測試站；P21／P22 維持未發布。
