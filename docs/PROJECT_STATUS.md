...
- PR #42：修正頭像拖移座標回傳欄位錯誤，並讓 Pointer capture 使用數字型 ID、保留舊版 Safari Touch fallback；生日表單、標籤與日期輸入套用完整 border-box／寬度限制，避免 iPhone 日期控制項超出卡片。StudentSettings 2 個測試與 Production build 已通過；Netlify Deploy Preview 已發布，仍待 iPhone Safari 實機驗收，尚未合併 `main` 或部署正式站。
