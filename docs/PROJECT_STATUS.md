...
- Workbook 1 A–Z 自動語音挑戰（尚未部署）：沿用 `codex/workbook1-speaking-challenges`，介紹頁字母格改為 `Aa`～`Zz`，且只有介紹頁可以播放單一 A–Z 主音檔。正式挑戰移除 3 秒倒數、指定字母提示音、手動開始／停止錄音與送出按鈕；同一輪只取得一次麥克風串流，以瀏覽器本機音量偵測逐題切出短音訊，自動送評並前進，完成、失敗、切到背景或離開時關閉。回到列表前新增「本輪紀錄不會存檔」確定／取消對話框。現有主音檔實查為 13 個 Puck 男聲與 13 個 Autonoe 女聲；新版管理操作固定使用女性 voice、沿用既有 13 個女聲來源並只生成缺少的字母，再重建單一主音檔，學生 API 會拒絕不是完整女聲 policy 的舊 sequence。全前端 54 suites／216 tests、題庫契約 22／22、Edge Function syntax 與 production build 已通過；尚未 commit、push、部署 Function 或固定測試站，也尚未產生缺少的女聲音檔。
