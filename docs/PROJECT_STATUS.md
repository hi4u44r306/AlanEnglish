...
- 管理員家長提醒再次寄送（本機完成，待測試站發布）：保留同一學生每日一般提醒只寄一次的預設限制；若今日已有成功寄送，只有管理員可開啟「再次寄送」，輸入 3～500 字原因並進行第二次確認。後端重新驗證管理員、今日原始寄送與目前家長 Email，使用獨立請求編號防止連點重複，另建寄送紀錄保存操作管理員、重寄原因及原信關聯。新增 additive migration `20260910020211_admin_guardian_email_resend_audit.sql`，只增加兩個 nullable 稽核欄位、限制與索引，不修改既有通知資料。家長寄信合約 8／8、管理頁測試 6／6、兩支相關 Edge Function 語法、Production build 及 `git diff --check` 均已成功；尚待套用測試 migration、部署 `guardian-email`／`learning-activity` 及固定測試站。正式站不變，未自動寄出測試信。
