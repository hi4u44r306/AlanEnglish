...
- Workbook 1 A–Z 自然女聲三候選已部署固定測試環境、待人工選音：因 Neural2-F 仍不夠自然，候選改為 Google Chirp 3 HD 的 Leda、Aoede、Zephyr。每個聲音各以一次完整 A–Z 請求產生單一 WAV，語速 0.92、字母間隔 800ms，Z 以 IPA 固定為美式 `zee`；不再拆成 26 次生成。Chirp 不提供現行可用的 SSML mark timepoint，因此後端會從 LINEAR16 WAV 偵測 25 個長靜音邊界，少一個就 fail closed，不建立可核准候選。三個 voice 與 revision 都由 server allowlist 固定，候選存私人 R2；管理員必須個別完整播放後才能核准其中一個，產生候選不會改學生目前版本。`speaking-tts-manager` v15、`speaking-content-manager` v19、`speaking-challenge` v20 均為 ACTIVE；固定測試站 deploy `6aa6b1fd06b9dd37b7b76a53` 已載入本批前端。真實候選已成功建立：Leda 36.660 秒／1,759,740 bytes、Aoede 39.846 秒／1,912,668 bytes、Zephyr 45.678 秒／2,192,604 bytes，三筆均為 `ready`、私人檔案存在且各有 26 段；沒有候選被啟用，學生目前 sequence 仍為原本 44.367 秒版本。無新 migration、無 P21／P22 內容變更。相關 helper／題庫契約 29／29、管理 UI／service 13／13、全 React 54 suites／216 tests、Edge syntax、production build 與 `git diff --check` 通過；待管理員特別試聽 I、J、K、L、N、R、S、V、Z 後選定一個候選。
