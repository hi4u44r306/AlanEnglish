...
- 學生口說大挑戰的「遊戲規則」改為預設收合，只保留一列清楚的查看按鈕；學生需要時點擊才展開「選一關、看題目、開口說」三步驟，再次點擊可收起。控制項提供 `aria-expanded`、鍵盤 focus、展開方向提示及手機至少 44px 的觸控範圍；老師與管理員唯讀預覽仍不重複顯示學生規則。只修改前端列表與測試，無 migration、無 Edge Function 變更。
