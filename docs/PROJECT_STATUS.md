...
- 口說大挑戰角色與題目頁視覺重建（測試站已部署，正式尚未部署）：移除先前過於簡化的向量人物，改用一致的金髮碧眼男孩／女孩高品質卡通角色；實際連結男聲或女聲示範語音時自動切換，播放期間才有輕微呼吸、動嘴與聲音柱動畫，並保留 reduced-motion。題目頁重整為進度列、角色引導卡及主題目卡，避免原本像未完成版的樸素版面。commit `572c1c3` 已推送，固定測試站 deploy `6aa27ff831ce6a977b86c41c` 已上線；已用登入中的在校學生確認男聲第 1 題顯示男孩、女聲第 2 題顯示女孩、播放期間人物轉為說話狀態，且學生桌面「AI 練習」下拉選單同時有 AI 教材與口說大挑戰。前端測試 11/11、Production build 與 `git diff --check` 通過；瀏覽器自動化的 412px viewport override 未生效，仍待 iPhone Safari 實機驗收。
...
- 口說大挑戰男女聲口說夥伴（測試站已部署，正式尚未部署）：改為一組一致的可愛男孩／女孩向量角色，學生端從真正連結到該題的 `speaking_tts_assets.voice_id` 判別性別；Chirp 3 HD 的 Autonoe／Leda 等女聲配對女孩，Puck 等男聲配對男孩。人物固定在題目旁，朗讀時才自然呼吸、動嘴、眨眼及顯示聲音動態，結束即靜止；保留 reduced-motion 與手機版版面。學生挑戰測試與 Edge Function 語法檢查已通過，`speaking-challenge` 與固定測試站已更新；尚待已登入學生帳號的視覺驗收。
