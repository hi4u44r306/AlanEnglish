...
- 教材口說大挑戰第一階段（尚未部署）：已建立學生專用路由 `/student/speaking-challenges`，每本教材的已發布題庫成為可進入的小關卡；只有有效且包含 `pronunciation` 的學生能透過新的 Firebase Token 驗證 Edge Function 讀取內容。學生可查看中文提示、示範回答、瀏覽器朗讀與發音重點，並以「我已開口練習」保存自己的完成紀錄；此階段不保存錄音、不呼叫評分服務、不發 XP／AE Points。新增 additive migration 的進度表已開啟 RLS 並撤銷前端直接存取，尚未套用遠端。下一階段才把題目接入錄音、AI 示範與逐字評分。
