...
本次進行中（2026-09-10，待部署測試站，正式尚未部署）：

- 口說大挑戰題庫瀏覽與資源收斂（待部署測試站，正式尚未部署）：依使用者回饋完全移除口說夥伴元件、三張人物 PNG（共約 5.4 MB）及所有人物動畫；題目保留一次一題、私人 R2 自然示範語音與需要時才出現的 SVG 圖卡。題庫目錄依 commit `9697939` 的設計恢復「依教材／依主題」切換與分組卡片，但沒有回退舊錄音、計分或權限程式。題庫與 Navbar 測試 12/12、Production build、`git diff --check` 已通過；待固定測試站部署與登入學生視覺驗收。
