...
- 公開首頁折疊版 Navbar 已新增常駐「登入」按鈕，並進一步把 ALAN ENGLISH、登入與漢堡按鈕固定為同一列；窄螢幕只縮小品牌字樣，登入及選單仍維持 44px 觸控高度。使用者在平板與手機寬度不必先打開漢堡選單即可找到登入頁；抽屜內原登入入口仍保留，桌面寬版導覽不變。本批不修改登入流程、Firebase、Supabase、權限、套件或產品資料。
