...
本次教材 AI 口說題庫緊湊化與語音重試保護（2026-09-17，本機驗證完成，尚未部署）：
...
- React 相關測試 2 suites／12 tests 與 Production build 已通過；build 僅有既有 `FiSquare` 未使用、Browserslist 與 Node deprecation 警告。同步修正早已過期的 P26～P27 第六個「建立草稿」按鈕測試斷言。本批沒有 migration、RLS、Edge Function、題庫資料、私人圖片或學生進度異動；尚待 `git diff --check`、正式發布與登入後桌面／手機驗收。
