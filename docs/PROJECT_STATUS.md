...
本次待發布草稿可變答案語音驗證修正（2026-09-23，本機完成、尚未部署）：
...
- 不需 migration，不修改、核准或發布任何現有草稿。TTS 規則、管理頁 18/18、Edge syntax、Production build 與 `git diff --check` 皆通過；完整口說契約 31/32，唯一失敗仍是既有手機導覽 CSS selector 斷言，與本批無關。未經明確指示不推送或部署。
