...
本次 Workbook 1 新版學生版 OCR 核對防錯（2026-09-24，本機分支 `codex/workbook1-ocr-review-guard`，尚未部署）：
...
- 管理頁 React 23/23、新頁碼檢查 2/2、Production build 通過；新增 OCR 契約 1/1，完整口說契約 35/36，仍只失敗於既有手機播放器 CSS selector 斷言。未部署新 Supabase Function，因而既有 12 批 OCR 不會自動重跑，也尚不能依 119 頁建立正確圖片關卡。下一階段需在隔離環境驗證新版 OCR 與逐題圖片對位後，取得本批 Function 部署授權，使用新版學生 PDF 重跑來源；教師版僅供教學意圖核對，人工審核後才建立私人草稿，不自動發布。
