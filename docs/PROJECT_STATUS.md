...
- 家長通知寄送狀態修正（本機完成，待部署測試站）：測試管理員直接寄送時，Edge Function 會先把通知改為 `sending`，但既有 `notification_logs_status_check` 只允許 `draft`、`sent`、`failed`、`cancelled`，因此請求尚未呼叫 Resend 就被資料庫拒絕；另因 Supabase 錯誤物件不是標準 `Error`，畫面只顯示通用錯誤。新增 additive migration `20260910002545_guardian_email_sending_status.sql`，只把 `sending` 納入既有狀態限制，不修改或刪除通知資料；`guardian-email` 同步保留安全的後端錯誤文字。寄信合約 5／5、全部 Edge Function 語法及 Production build 已成功，待套用測試 migration、部署測試 `guardian-email` 並驗證限制與未登入防護；尚未再次寄送任何信。
