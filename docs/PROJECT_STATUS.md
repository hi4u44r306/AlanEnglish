...
- 口說大挑戰手機單屏緊湊版（本機已完成，尚未部署）：在 `760px` 以下把關卡頁改成扣除 66px Navbar 的單一動態視窗工作區，教材標頭縮為返回鍵、單行名稱與進度，題目、三段頁籤、個人答案、示範／提示及錄音器皆降低非必要留白但保留至少 44px 觸控區。一般手機不再整頁上下捲動；極短螢幕或長內容只在題目卡內安全捲動。口說詳細頁隱藏會覆蓋操作區的浮動「今日作業」，返回其他頁即恢復。相關 5 suites／11 tests、全前端 46 suites／147 tests、production build 與 `git diff --check` 均成功；尚待部署後在 412×915、681×919 與 iPhone safe area 驗收。
