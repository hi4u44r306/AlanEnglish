...
- 有效聆聽覆蓋率與平板播放器可靠性修正（尚未部署）：正式畫面曾出現音檔播放至 `1:40 / 1:41`，但有效聆聽只有約 64%。排查確認前端舊版只接受單次最多 3 秒的 `onListen` 差值；瀏覽器事件被延後時會整段丟棄，且第一首音檔若播放當下 metadata 尚未就緒，伺服器 listening session 不會自動補建。分支 `codex/fix-player-coverage-and-tablet` 已改為以音檔前進量對照單調實際時間、在 `canplay`／後續播放事件重試 session、頁面隱藏即暫停，並讓未達 80% 的完成請求保存為不可發獎診斷，不增加次數、XP、AE Points 或作業進度。播放器自訂樣式改在第三方樣式之後載入，`900px`（含）以下統一使用緊湊排列，涵蓋 `682px`～`810px` 擠壓區。聽力獎勵契約 14/14、新 coverage 單元測試 4/4、全前端 37 suites／106 tests、11 支 Edge Function 語法、Production build 與 `git diff --check` 均成功；待部署 `record-play` 與正式網站後，驗收 `412、682、768、810、900、1024px`、播放／切換／展開、Sidebar、今日作業與 safe area。
