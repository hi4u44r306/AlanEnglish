...
- `codex/fix-mobile-avatar-notifications`：生日欄位在 560px 以下改為垂直排列，避免與儲存按鈕重疊；頭像裁切視窗保留桌面 Pointer 拖移並新增 iPhone Touch 拖移。通知改為受保護的獨立 `/student/notifications` 頁面，可標示已讀與分頁載入較早通知；Header／手機鈴鐺均導向新頁面。`membership-manager` 的 notifications action 仍會從 Firebase 驗證後的呼叫者取得 `student_id`，只新增 1～100 筆的安全分頁與到期通知過濾。本機測試與 Production build 已通過，尚未推送、部署 Netlify 或部署 Edge Function。
