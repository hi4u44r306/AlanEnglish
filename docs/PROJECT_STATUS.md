...
- 學生設定方案狀態修正：`codex/fix-departed-ai-addon` 已在本機完成。離校生會從最近一次 enrollment 顯示實際離校日；「基本會員與 AI 方案」不再混入歷史 `academy_internal` 在學權限，已到期／撤銷方案也不再誤顯示續訂日。AI 加購名稱與權限提示統一為「AI 教材與發音練習」，明確包含 AI 教材生成及發音練習。`StudentSettings` 6/6 與 Production build 通過；尚未 push、部署測試站或正式站。
