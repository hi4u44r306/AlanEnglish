...
- 已定位老師頁找不到 Workbook 1 的原因：教材下拉選單錯把班級目前生效教材設定當成螺旋複習題庫的可用範圍。修正版保留老師／管理員的班級發布權限，但教材改列出所有已有人工核准逐頁字卡的正式教材；首次載入會自動選取第一本可出題教材，切換班級不再把 Workbook 1 換掉。學生原有教材 entitlement 與班級教材設定均不變。本批不需要 migration；尚待測試、推送、部署 `spiral-review` 與固定測試站。
