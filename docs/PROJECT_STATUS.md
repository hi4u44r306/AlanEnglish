...
- 測試與部署：題庫與 Navbar 前端測試 12/12、Production build、`git diff --check` 通過；commit `4027dbc` 已推送，固定測試站 deploy `6aa2a6fda35aa67687a34936` 已上線。登入中的在校學生已驗收：人物不再渲染、Workbook 1／2 分組與生活主題切換正常。正式 `main` 尚未合併；PR 待本次紀錄完成後建立。
