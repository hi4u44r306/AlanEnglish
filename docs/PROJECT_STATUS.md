...
- 聽力 XP／AE Points、純聽力作業與升等經濟規則（本機實作完成，尚未部署）：additive migration `20260902021837_listening_rewards_and_level_up.sql` 已改為不需付費 Supabase branch 的正式庫灰度架構；`student_feature_rollouts` 缺少資料時預設關閉，只有明確加入 `listening_rewards_v2` 的專用測試學生才走新流程，其他正式帳號繼續使用舊流程。新規則把一般總聽力與作業進度建立在同一筆 80% 有效 Session，但作業只累計發布後至截止前的 `assignment_listening_events`；發布前的終身播放次數不會直接完成新作業。同一次有效聽力可同時推進多份符合條件的作業，但 XP／AE Points 只結算一次。新作業僅允許 `music_track`，每檔 1～10 次、預設 3 次，整份完成一次性發 30 XP／5 AE Points；舊 AI 作業只保留教師歷史報表，學生不再收到或提交。使用手冊已更新為 v1.23 並明示尚未啟用正式灰度。聽力獎勵契約 11/11、商務契約 37/37、全前端 36 suites／102 tests、11 支 Edge Function 語法、Production build 與 `git diff --check` 均成功。因本批不建立付費測試 branch，SQL 尚未在真實 Postgres 套用演練；本批也尚未改動任何遠端 Supabase 或正式網站。
