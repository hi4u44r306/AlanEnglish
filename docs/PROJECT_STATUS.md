...
- 口說大挑戰題庫瀏覽與圓角修正：恢復「依教材／依主題」切換與 Workbook／生活主題分組；分組卡片使用 `overflow: hidden` 與 22px 外框圓角，避免深藍教材標頭及小關卡格線切破外圍圓角，手機版則改成單欄。題目卡維持 26px 圓角，圖卡維持 16px 圓角。
- 題庫圖卡與內容校正：已盤點 6 組、36 題題庫，為 15 題加入受白名單限制的 SVG 圖卡；國旗、顏色物品、時鐘與日常情境只提供作答線索。時間題改為先看時鐘再回答；管理端可設定與預覽圖卡，migration `20260910090000` 已套用。
- 學生導覽與登入：公開首頁手機／窄螢幕恢復直接登入快捷鈕；登入學生的桌面 Navbar 將「AI 教材」與「口說大挑戰」收進「AI 練習」下拉選單，手機側邊欄仍保留直接入口。
- 資源收斂：今日曾試作語音搭配人物引導，但最終已依使用者回饋移除人物元件、動畫與三張 PNG（約 5.4 MB）；保留私人 R2 自然示範語音、錄音與 AI 評分流程，以及需要時才顯示的圖卡。
- 整合原則：合併最新 `main` 時發現它已包含較新的錄音、AI 評分與成本控制功能；本分支以 `main` 為底整合，保留這些功能，只疊加題庫分組、圓角、圖卡與導覽調整，不再以舊分支覆蓋既有功能。
- 驗證與發布：題庫與 Navbar 前端測試 13/13、Edge Function 語法檢查、Production build、`git diff --check` 通過；已完成最新 `main` 的整合 commit `52fed53`，固定測試站 deploy `6aa2bbd366cfd0b00cf2744d` 已上線。登入在校學生已確認 Workbook 1／2 分組與生活主題切換正常；PR #104 已解除衝突並等待 Netlify Preview 檢查完成，正式 `main` 尚未合併。
