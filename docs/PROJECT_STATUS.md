...
- `codex/student-access-overview`：已完成學生主要路由與 412px 手機 Sidebar 的正式站巡查；確認原側欄同時放入功能、成果、18 本教材與帳號入口，且會員頁沒有回答「目前可用哪些教材／功能」。本機已將 `/student/membership` 改為「我的教材與功能」，使用既有 `membership-manager` 有效功能與 `content-access` 教材權限顯示可用／鎖定狀態；學生桌面導覽縮減為首頁、教材與功能、英文對話、AI 教材及全部功能，手機側欄改為開始學習／學習成果／帳號與支援，並移除逐本教材的第一層清單。Dashboard 也停止顯示舊版 `allcover`／`listeningonly` 方案名稱，無班級學生的排行標題改為不誤導的「學習排行榜」。MembershipCenter／MainNavbar 共 7 項測試、`git diff --check` 與 Production build 已通過；尚未 push、PR、部署或以新版頁面進行正式站登入視覺驗收。
