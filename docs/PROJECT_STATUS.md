...
本次 iPhone Web Push 裝置自測（2026-09-26，功能分支實作中，尚未部署）：

- 正式站出現 1 筆近期建立的啟用中 Apple Push 訂閱，與使用者回報 iPhone 17 Pro Max 已開啟的時間相符；尚未發送或收到推播。`codex/web-push-self-test` 將在通知頁加入手動測試按鈕，後端僅對已驗證學生本人在此裝置的訂閱送出低敏感度測試訊息；測試寫入站內系統通知與推播佇列紀錄，與正式提醒共用安靜時段及每日三則上限，同分鐘防重複。測試不建立假作業，也不向班級群發。實機顯示與點擊仍待使用者驗收。
