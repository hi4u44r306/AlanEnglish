...
- 當天教材版本修正（本機完成、尚未部署）：分支 `codex/correct-current-class-materials` 以正式 `main` commit `5b77ee9` 為基準。正式 E3 已由管理員於 2026-09-01 成功建立第 2 版「2026 秋季」，教材為 Workbook 3、Super Easy Reading 3；3 位歷史學生取得 6 筆永久教材權限，1 位目前在校生受新版影響，既有有效作業 0 份。後續重選教材時因原 RPC 禁止同日第二版本而回傳 500，前端又將 PostgREST 錯誤物件誤顯示為「教材商務服務暫時無法使用」。本批新增「修正目前版本」模式：只允許管理員修正台北日期今天建立且目前生效的同一設定，可在同一天重複修正，每次須用最新 `updated_at` 重新預覽與二次確認；同一交易替換教材清單、更新學期名稱並寫入 `corrected` 前後快照，不建立重疊版本、不修改永久 entitlement、不刪除學習紀錄或作業快照。Edge Function 同步回傳真正資料庫錯誤與適當 HTTP 狀態。當天連續修正兩次的元件測試 1/1、教材管理頁 3/3、商務契約 35/35、全前端 34 suites／98 tests、Edge Function 語法、Production build 與 `git diff --check` 均成功。Additive migration、`commerce-manager`、React 管理頁與網站手冊尚未部署；正式 E3 目前資料未被本批修改。
