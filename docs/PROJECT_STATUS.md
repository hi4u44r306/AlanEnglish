...
- 管理員 API 成本控制中心整合（本機整合中，正式尚未部署）：把原先只存在 `codex/admin-api-cost-dashboard` 的互動式月用量儀表板帶入目前最新的 `codex/speaking-guided-practice`，避免後續口說整包部署再次覆蓋。`/admin/api-usage` 將整合 OpenAI AI 教材紀錄、口說 OCR／題庫生成 Token 與 Google Cloud TTS 字元數，顯示總預算進度、月底預估、每日趨勢、成功率及預算／失敗率／單日突增警示；Supabase、Cloudflare R2、Netlify、Firebase Authentication、Resend 與 PAYUNi 維持「外部核對」，未知帳單不誤顯示為零元。本批不新增 migration，需重新部署 `generate-ai-material` 與測試站前端後驗收。
