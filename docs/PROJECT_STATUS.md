...
- P2 好友／公開暱稱與通知同步（本機完成，尚未推送或部署）：通知頁單筆或全部標示已讀時，以前端事件同步 Navbar 徽章；學生暱稱新增前後端不當公開內容防護、正規化後重複檢查與友善提示。會員 Profile、遊戲化摘要及排行榜帶回公開暱稱，學生 Navbar、首頁、設定摘要與排行榜優先使用暱稱，未設定才回退帳號姓名；教師班務、客服與帳務仍維持真實姓名。好友、通知與 Navbar React tests 15／15、社交安全契約 4／4、全部 Edge Function 語法、`git diff --check` 與 Production build 均成功；待使用者核准後才可推送／部署 `membership-manager`、`gamification`、`student-social` 與測試站。
