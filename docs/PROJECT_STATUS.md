...
- 離校生歷史教材永久保留（本機完成，尚未部署）：分支 `codex/preserve-departed-class-materials` 新增 additive migration `20260901030505_preserve_departed_academy_materials.sql`。盤點會整合每段 enrollment 期間重疊的班級教材版本、班級作業教材及伺服器播放進度；離校 RPC 在同一交易內建立永久 `academy_history` `student_book_entitlements`、以排定生效日結束 enrollment，並記錄保留教材清單。Migration 會依相同日期邊界回填既有 `withdrawn`／`graduated` 離校紀錄，upsert 可重複執行。管理員離校預覽會列出永久保留教材，學生設定頁會獨立顯示來源。NT$299「基本自主學習會員」仍只恢復已擁有教材的聽力、會話與複習功能，不要求重買實體書、不贈送新教材；`content-access` 仍要求有效平台權限與逐本 entitlement，`assignment-manager` 仍只允許有效在校 enrollment 且方案具 `assignments`。正式 Supabase 已以唯讀聚合查詢驗證欄位型別、constraint／索引與核心盤點 SQL；目前 3 段離校 enrollment 中預估影響 2 位學生、寫入 4 筆 enrollment／教材 entitlement（共 2 本不同教材），未讀取或輸出個資。商務契約 28/28、學生設定 7/7、所有 Edge Function 語法、Production build 與 `git diff --check` 均成功。遠端 migration、`commerce-manager` 部署、正式資料回填、推送／PR／Netlify 部署及線上角色驗收尚未執行。
