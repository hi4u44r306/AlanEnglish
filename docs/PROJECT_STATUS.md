...
- 管理員家長提醒再次寄送（測試站已部署，正式站尚未部署）：保留同一學生每日一般提醒只寄一次的預設限制；若今日已有成功寄送，只有管理員可開啟「再次寄送」，輸入 3～500 字原因並進行第二次確認。後端重新驗證管理員、今日原始寄送與目前家長 Email，使用獨立請求編號防止連點重複，另建寄送紀錄保存操作管理員、重寄原因及原信關聯。additive migration `20260910020211_admin_guardian_email_resend_audit.sql` 已套用並登記測試 Supabase，兩個 nullable 稽核欄位已唯讀確認存在；`guardian-email` 與 `learning-activity` 已部署。家長寄信合約 8／8、管理頁測試 6／6、兩支相關 Edge Function 語法、Production build 及 `git diff --check` 均成功；固定測試站 deploy `6aa21773254c04a95014528d` 已上線，管理／學生首頁回應 200，兩支後端 OPTIONS 200、未登入 POST 401。正式站不變，未自動寄出測試信。
- 家長通知信品牌 Logo（測試站後端已部署，正式站尚未部署）：`guardian-email` 的家長週報、個別學習提醒與既有通知草稿寄送三種 HTML 模板，均在深藍標頭加入使用者選定的深藍底、白黃 AE 圖形 Logo、固定尺寸及替代文字；圖片沿用正式網站公開 HTTPS 資源，已確認回應 200 與 `image/png`。寄信合約 6／6、全部 Edge Function 語法、Production build 與 `git diff --check` 均成功；測試 `guardian-email` v25 為 ACTIVE，OPTIONS 200、未登入 POST 401。未為避免打擾家長而自動寄出測試信，仍待管理員以單一測試收件人驗收 Gmail 圖片顯示。Gmail 寄件者名稱旁的頭像因品牌尚無註冊商標或滿一年公開使用證明，暫時無法申請 CMC／VMC 並完成 BIMI；本批未修改 DNS。
- 家長通知寄送狀態修正（測試站後端已部署，正式站尚未部署）：測試管理員直接寄送時，Edge Function 會先把通知改為 `sending`，但既有 `notification_logs_status_check` 只允許 `draft`、`sent`、`failed`、`cancelled`，因此請求尚未呼叫 Resend 就被資料庫拒絕；另因 Supabase 錯誤物件不是標準 `Error`，畫面只顯示通用錯誤。新增 additive migration `20260910002545_guardian_email_sending_status.sql`，只把 `sending` 納入既有狀態限制，不修改或刪除通知資料；`guardian-email` 同步保留安全的後端錯誤文字。checkpoint `6d60659`；測試 migration 已套用並列入遠端歷史，constraint 已驗證包含 `sending`，測試 `guardian-email` v23 為 ACTIVE。寄信合約 5／5、全部 Edge Function 語法及 Production build 已成功；尚未再次寄送任何信，待管理員以單一測試收件人實際驗收 Resend 投遞結果。
