...
本次播放器專注模式（2026-09-15，待正式發布）：

- 學生從教材播放音檔後進入口說大挑戰，系統會暫停教材音檔但保留曲目與目前時間，並把原本占用版面的底部播放器收成 48px 音樂圖示；點圖示才可展開完整播放器。此模式避免教材音檔與麥克風練習同時播放，也不會清除有效聆聽進度。手機版播放器的教材摘要與上一首／播放／下一首控制鍵改採同一垂直對齊基準，修正 iPhone Chrome 快速捲動時控制列看起來分離的情況；播放器仍位於學生底部導覽上方並保留 safe area。無 Firebase、Supabase、R2、migration、Edge Function 或權限邏輯變更。
