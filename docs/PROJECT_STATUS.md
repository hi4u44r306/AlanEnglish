...
- 管理員 API 成本控制中心（本機完成，尚未部署）：`/admin/api-usage` 已改為互動式月用量儀表板，整合既有 OpenAI AI 教材紀錄、口說 OCR／題庫生成 Token 與 Google Cloud TTS 字元數，顯示總預算進度、月底預估、每日趨勢、成功率及預算／失敗率／單日突增警示。Supabase、Cloudflare R2、Netlify、Firebase Authentication、Resend 與 PAYUNi 以「外部核對」卡呈現，未知帳單不會誤顯示為零元。未新增 migration；需部署更新後的 `generate-ai-material` 與測試站前端後才能取得完整資料。
