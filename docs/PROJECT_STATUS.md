...
- `feature/assignment-effort-rewards`：新作業發布時，`assignment-manager` 依所有音檔的 `duration_seconds × 指定聆聽次數`，再加 90 秒開啟作業與每首 15 秒切換緩衝，將預估秒數與完成獎勵寫入作業；10 分鐘內為 30 XP／5 AE Points、11–20 分鐘為 40／7、21–35 分鐘為 55／10、超過 35 分鐘為 70／14。缺少任一音檔長度時不估造分鐘數，學生端顯示「暫無法估算時間」並維持基本獎勵。`ae_try_grant_assignment_completion` 改為讀取已保存獎勵，依既有 ledger source key 只發一次。老師發布預覽與歷史、學生 Dashboard、作業頁均顯示相同資料；既有私有學生頭像上傳及排行榜顯示功能已確認保留。2026-08-26 已透過 Supabase migration API 套用 `assignment_effort_rewards`（遠端版本 `20260826003156`），`assignment-manager` v20 已部署並為 ACTIVE，OPTIONS 健康檢查回應 200。尚需使用明確標示的沙盒老師／學生帳號驗證發布、完成、ledger 僅入帳一次與桌機／412px／iPhone Safari 實機畫面。
