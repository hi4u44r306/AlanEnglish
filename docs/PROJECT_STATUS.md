...
本次口說大挑戰 Workbook 第一層冒險卡片（2026-09-17，本機驗證完成，尚未部署）：
...
- `TextbookSpeakingChallenge.test.jsx` 22/22、Production build 與 `git diff --check` 均通過；新增教材配色、可存取名稱、進度值、行動文字與路由回歸測試。本機桌面版已確認兩本教材使用不同配色、卡片可正常進入原 Workbook 關卡列表且版面無重疊；build 只有既有 `FiSquare` 未使用、Browserslist 與 Node deprecation 警告。正式站發布仍待完成。
