...
- Workbook 1 A–Z 發音發布阻擋修正：管理員已建立題組 `id=7`，26 題完整但仍為 `draft`。第一次「產生／補齊 A–Z 標準發音」已把 26 個 WAV 寫入私人 R2，最後連結題目時因遠端 `speaking_question_audio` unique key 與舊 `onConflict` 不一致而全部標成 `42P10`／failed，故學生 catalog 正確排除該草稿。`speaking-tts-manager` v11 已改為不依賴特定 constraint 的 update／insert／race retry，並只在 R2 明確 404 時才允許重新生成；現有 26 個音檔可用 HEAD 與大小核對後直接恢復連結，不再次產生付費語音。`speaking-content-manager` v16 另鎖定 A–Z 通用編輯，發布前逐題驗證固定 A–Z、0～25 順序、答案及 26 個 ready 音檔來源。兩支 Function 均為 ACTIVE、`verify_jwt=false` 且無憑證 POST 回應 401；P0／P1 為 0。本次尚未替管理員執行音檔恢復或發布，題組仍須在管理頁再次按「產生／補齊 A–Z 標準發音」，確認 26/26 後再人工發布；P21／P22 未變更。
