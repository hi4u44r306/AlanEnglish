...
- 口說大挑戰角色與題目頁視覺重建（尚未部署）：移除先前過於簡化的向量人物，改用一致的金髮碧眼男孩／女孩高品質卡通角色；實際連結男聲或女聲示範語音時自動切換，播放期間才有輕微呼吸、動嘴與聲音柱動畫，並保留 reduced-motion。題目頁重整為進度列、角色引導卡及主題目卡，避免原本像未完成版的樸素版面；待完成前端測試、Production build、桌面與手機視覺驗收後才可部署測試站。
