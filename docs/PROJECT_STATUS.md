...
- 學生口說大挑戰固定順序與角色預覽：學生列表改依教材關卡編號排序，完成前一關才會開啟下一關；前端鎖定卡與 `speaking-challenge` 後端網址保護一致，不能透過直接網址跳關。老師／管理員可從 Navbar 的「口說大挑戰預覽」唯讀開啟全部已發布關卡，不會寫入進度或獎勵。手機版口說列表與詳細頁收起 Logo Header，縮小頂部留白；桌面版保留完整 Navbar。A–Z 介紹頁在超過手機寬度時也採 5 欄大卡片，字級提高至 34–48px，手機版既有 5 欄與尺寸不變。功能 commit `c29c1e0` 已推送 `feature/speaking-challenge-progression`，stacked PR #127 已建立；相關 React 26/26、progression contract 3/3、Edge Function syntax、Production build 與 `git diff --check` 均成功。共用 Supabase `speaking-challenge` 已部署；最新固定測試站 deploy `6aa819d6d61b365587165993` 與正式站 deploy `6aa81a24f0c8eb4010c6c4a6` 已就緒，首頁與 `/student/speaking-challenges` 都回應 HTTP 200，並載入桌面 Navbar hotfix CSS bundle；Function CORS preflight 回應 200。本批沒有 migration。
