...
- 聽力 XP／AE Points、防掛機與升等經濟規則（本機實作完成，尚未部署）：已建立 additive migration `20260902021837_listening_rewards_and_level_up.sql`，把 80% 有效聆聽、單一進行中 Session、同音檔同台北日防重、每日前 10 首各 +5 XP、每 5 首 +1 AE Point、升等一次性點數、作業高分僅加 XP、遊戲每日最多 2 點及實體獎品 30 天冷卻收在後端原子流程；舊 7 次／100 次門檻不再是獎勵條件。`record-play` 改由後端 RPC 建立與完成工作階段，仍重新驗證 Firebase、有效會員、教材權限、伺服器音檔長度、不重複覆蓋區段及最短經過時間。播放器已加入背景不累計、加速工作階段永久失格、連續 15 分鐘或自動 5 首注意力確認、每日獎勵進度、同音檔重播另開一般進度 Session、一般獎勵提示及可關閉升等動畫；Navbar 會在結算後刷新 XP／點數。試用會員可累積但前後端都禁止兌換，獎品後台可區分實體／數位類型。使用手冊已更新為 v1.22 並明示尚未部署。獎勵契約 8/8、全前端 36 suites／102 tests、11 支 Edge Function 語法、Production build 與 `git diff --check` 均通過；尚待在隔離 Supabase 資料庫實際套用 migration、驗證原子發獎／防重／回復，再進行 PR、正式部署與帳號驗收。本批尚未改動任何遠端 Supabase 或正式網站。
