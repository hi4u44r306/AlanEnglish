...
- 口說大挑戰發音評分第二階段（本機完成，尚未推送／部署）：每題新增麥克風錄音、回聽、16 kHz 單聲道 PCM WAV 轉換、Azure Pronunciation Assessment 與逐字綠／黃／紅結果；後端只接受 `question_id`，自行從已發布題庫讀取完整示範答案與學生 AI 發音權限。新增 additive migration `20260904161000_speaking_pronunciation_attempts`，只保存分數、辨識文字與逐字結果，絕不保存原始錄音；每位學生 10 分鐘最多 12 次送評。下一步為本機測試、build 與使用者授權後的測試環境 migration／Edge Function／網站部署。
