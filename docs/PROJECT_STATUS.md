...
- Google Cloud 自然示範語音（本機完成，尚未部署）：新增 additive migration `20260904072331_speaking_question_audio_assets`，以伺服器專用、RLS 啟用且撤銷 `anon`／`authenticated` 的兩張表保存去重語音資產及題目連結；`speaking-tts-manager` 僅允許 Firebase 驗證後的管理員呼叫，依示範回答、Chirp 3 HD Voice ID 與輸出設定計算 SHA-256，相同內容沿用，否則以 Service Account OAuth 呼叫 Google Cloud Text-to-Speech、驗證回傳檔案並存入私人 R2。預設聲音為 `en-US-Chirp3-HD-Leda`；發布題庫後會自動產生語音，已發布題庫可補產生。學生端不再使用裝置 `speechSynthesis`，只播放 `speaking-challenge` 回傳的 15 分鐘短效 R2 URL，缺少資產時顯示「語音準備中」。尚未設定 `GOOGLE_CLOUD_TTS_SERVICE_ACCOUNT_JSON`／可選的 `GOOGLE_CLOUD_TTS_VOICE_NAME`，也尚未套用 migration、部署函式或產生付費語音。
