...
本次進行中（2026-09-10，測試站已部署，正式尚未部署）：

### 本日完整變更紀錄（2026-09-10）

- 口說大挑戰視覺與流程（commit `654115f`、`c2b9dac`、`572c1c3`）：先完成自然示範語音的播放狀態、一次一題、題號與上一題／下一題切換；切題會停止前一題語音。今日中段曾試作男／女聲卡通人物與說話動態，但依使用者資源負擔回饋，最終版本已完整移除，並不再下載人物素材或執行人物動畫。
- 題庫圖卡與內容校正（commit `8fee430`）：已盤點 6 組、36 題題庫，為 15 題加入受白名單限制的 SVG 視覺輔助；國旗、顏色物品、時鐘與日常情境只提供作答線索。時間題改為先看時鐘再回答，避免題幹直接說出答案；管理端可設定與預覽圖卡，migration `20260910090000` 已套用。
- 學生導覽與首頁登入（commit `c388d37`、`c2b9dac`）：公開首頁手機／窄螢幕重新提供直接登入快捷鈕；登入學生的桌面 Navbar 將「AI 教材」與「口說大挑戰」收進「AI 練習」下拉選單，手機側邊欄仍保留直接入口。
- 題庫卡片、圓角與分組復原（commit `4027dbc`）：依 commit `9697939` 的舊設計，重新提供「依教材／依主題」切換，並以 Workbook／生活主題分組。分組卡片用 `overflow: hidden` 搭配 22px 外框圓角，避免藍色教材標頭與兩欄小關卡格線切破外圍圓角；手機版改為單欄。題目卡保留 30px 圓角，圖卡保留 16px 圓角。
- 測試與部署：題庫與 Navbar 前端測試 12/12、Production build、`git diff --check` 通過；commit `4027dbc` 已推送，固定測試站 deploy `6aa2a6fda35aa67687a34936` 已上線。登入中的在校學生已驗收：人物不再渲染、Workbook 1／2 分組與生活主題切換正常。正式 `main` 尚未合併；PR #104 已建立，待審查與合併。

- 口說大挑戰題庫瀏覽與資源收斂（測試站已部署，正式尚未部署）：依使用者回饋完全移除口說夥伴元件、三張人物 PNG（共約 5.4 MB）及所有人物動畫；題目保留一次一題、私人 R2 自然示範語音與需要時才出現的 SVG 圖卡。題庫目錄依 commit `9697939` 的設計恢復「依教材／依主題」切換與分組卡片，但沒有回退舊錄音、計分或權限程式。commit `4027dbc` 與固定測試站 deploy `6aa2a6fda35aa67687a34936` 已上線；登入在校學生已確認人物不再渲染、教材分組（Workbook 1／2）與主題分組皆可切換。題庫與 Navbar 測試 12/12、Production build、`git diff --check` 已通過；仍待 iPhone Safari 實機 safe area 驗收。

- 口說大挑戰版面與學生 Navbar 收斂（測試站已部署，正式尚未部署）：原本會同時渲染題庫全部小題，加入人物與圖卡後容易造成一頁堆疊、閱讀焦點混亂；已改為一次顯示一題，使用題號、上一題／下一題切換，切題時會停止前一題示範語音。學生桌面 Navbar 將「AI 教材」與「口說大挑戰」收進新的「AI 練習」下拉選單；手機側邊欄維持直接學習入口。相關前端測試 11/11、Production build 與固定測試站 bundle／路由驗收均通過。
- 公開首頁手機登入快捷鈕（測試站已部署，正式尚未部署）：已恢復品牌旁、漢堡選單左側的 44px 直接登入按鈕；仍保留側邊欄內登入，390px 與 340px 以下會縮小品牌與按鈕間距避免重疊。新增 Navbar 測試確認直接連結仍為 `/login`；固定測試站已驗證最新 bundle 含登入快捷鈕。
...
- Google TTS 國小生語速與清晰度修正（測試站已部署，正式尚未部署）：保留 `en-US-Chirp3-HD-Leda`，將示範語速由原生 `1.0` 降至 `0.82`，輸出由壓縮 MP3 改為未壓縮 LINEAR16 WAV，設定版本更新為 `elementary-clear-v3`。新設定雜湊會建立並連結新版資產，舊 MP3 保留不覆蓋；additive migration `20260904145405_speaking_tts_wav_quality` 已套用並查證格式 constraint 同時接受 MP3／WAV。`speaking-tts-manager` 已部署為 ACTIVE v3，未登入請求正確回 401；固定測試站 deploy `6a9adbf6b323c700c514fe55` 回應 200。下一步由管理員補產生少量題目，真人比較新版語速與清晰度。
...
- 發布流程可靠性修正：專案不再依賴電腦上可能過期的全域 CLI。`package.json` 新增 `release:preflight`（checkpoint 前）及 `release:deploy-preflight`（部署前）檢查功能分支、未提交檔案、遠端 upstream、ahead／behind 與 Firebase Token 驗證設定；測試站的 `deploy:speaking-test` 固定使用 Supabase CLI `2.116.0` 與 Netlify CLI `27.4.3`。本機 OCR 暫存目錄 `/tmp/` 已忽略，不會被誤加入 commit。每次發布順序為：完成修改 → `npm run release:preflight` → commit／push → `npm run release:deploy-preflight` → 執行對應部署命令；若工具或 Git 狀態不符合，會先停止並指出原因。
