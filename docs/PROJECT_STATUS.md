...
本次 Workbook 1 新版學生版 OCR 核對防錯（2026-09-24，Supabase Function 已部署；前端尚未部署）：
...
- 管理頁 React 23/23、新頁碼檢查 2/2、Production build 通過；新增 OCR 契約 1/1，完整口說契約 35/36，仍只失敗於既有手機播放器 CSS selector 斷言。使用者同意直接部署後，正式 `speaking-content-manager` v52 已為 ACTIVE；OPTIONS 200、未登入 POST 401，未進行 migration。Supabase 目前方案拒絕建立隔離分支（Branching 需 Pro），沒有測試分支或資料異動。localhost Chrome 上傳新版學生 PDF 時，擴充功能因未啟用本機檔案 URL 存取而拒絕選檔；新文件未建立，OCR 未開始，舊 12 批與草稿均未覆蓋。待使用者啟用瀏覽器擴充功能的檔案存取後，先重辨識 P1–P10 檢查品質，再繼續其餘批次。教師版僅供教學意圖核對；仍須人工核對圖片及文字，不自動核准或發布。
