...
- 修正 P14～P17 第一題發音已由 `pronunciation-coach` 依兒童容錯判定通過後，`speaking-challenge` 在保存完成進度時又用舊版嚴格轉寫規則重判，造成 409 並讓學生看到「口說大挑戰暫時無法開啟」的問題。完成服務改讀取最近一次由伺服器保存的 `answer_match`，不接受前端傳入正誤值，因此兒童容錯判定與進度保存一致，漏字／多字／換序仍不會通過。無 migration；只需部署 `speaking-challenge` Function，P21／P22 維持未發布。
