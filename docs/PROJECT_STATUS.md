...
- 已修正老師頁找不到 Workbook 1：教材下拉選單原本錯把班級目前生效教材設定當成螺旋複習題庫的可用範圍。新版保留老師／管理員的班級發布權限，但教材改列出所有已有人工核准逐頁字卡的正式教材；首次載入會自動選取第一本可出題教材，切換班級不再把 Workbook 1 換掉。學生原有教材 entitlement 與班級教材設定均不變。本批不需要 migration。Checkpoint `df6157a` 已推送並更新 PR #109；`spiral-review` v4 為 ACTIVE，未登入 POST 回應 401。固定測試站 deploy `6aa37ec388ac3bcdef3824ff` 已發布，線上老師頁回應 200 並載入 `main.09fb53d0.js`；以已登入管理員實機重新整理後，教材選單已自動顯示「Workbook 1（可出題）」及 119 頁核准資料。契約 4／4、頁面測試 2／2、全部 Edge 語法、Production build 與 `git diff --check` 均成功。
