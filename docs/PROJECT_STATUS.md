...
本次進行中（2026-09-15，尚未部署）：

- Workbook 1 P14～P17 看字拼讀的兒童容錯判定：保留字母數量與順序必須完全正確、漏字／多字／換序仍不通過；當 Azure 整句轉寫不可靠時，後端會改核對所有候選中的逐字母對齊結果，以平均清楚度 45、單字母最低 20 作為兒童發音容錯。逐字母已對齊但分數不足或資料不完整時標記為「系統沒有聽清楚」，不算學生答錯、不跳題也不保存完成，並使用練習提示音請學生把字母稍微分開再試。A–Z 關卡、題庫答案、Firebase／教材 entitlement、原始錄音不保存與費用限制均維持不變；無 migration。分支 `fix/p14-p17-child-tolerant-recognition`，尚未部署 `pronunciation-coach` 或前端。
