...
- 口說大挑戰完成獎勵與動畫（本機完成，待測試部署）：學生首次完成同一個已發布題庫的全部題目時發 30 XP，有效在校英文班學生另得 3 AE Points；完成、點數資格、升級與帳本寫入由資料庫交易一次處理，並以 advisory lock 及既有唯一獎勵來源阻擋連點、重試、重錄與教材／主題雙入口造成的重複發獎。前端只在後端回傳首次發獎成功時顯示完成動畫、獎勵與升級結果，不能自行決定獎勵數量；老師與管理員示範仍不得寫入進度或領獎。全前端 51 suites／175 tests、獎勵契約 4／4、角色唯讀契約 3／3、全部 Edge Function 語法、production build 與 `git diff --check` 均成功；additive migration、`speaking-challenge` 與測試站尚待本次 checkpoint 後部署。
- 口說大挑戰入口與私人錄音播放器精緻化（本機完成，尚未推送或部署）：入口可在「依教材」與「依主題」之間切換；教材改為 Workbook 大關卡包住 01、02、03…小關卡，主題則依姓名與自我介紹、日常對話、校園英語、家人與人物、位置與問路、生活物品與顏色等生活情境重新整理，同一題庫只改瀏覽方式、不複製進度。口說練習與口說歷程共用品牌錄音播放器，可播放／暫停、拖曳進度並顯示時間，送評後仍能回聽自己的錄音。學生按下第一個對話／錄音操作時會先解鎖瀏覽器音效，再於評分完成後播放更清楚的成功、再練習或重試提示音，改善手機自動送評後沒有聲音的情況；裝置靜音與系統媒體音量仍須由使用者自行調整。口說完成獎勵尚未實作；建議下一階段由後端以每個已發布大關卡為冪等來源，首次完成全部題目發 30 XP，有效在校英文班學生另得 3 AE Points，工作人員不發獎，且不得用重錄或高分重複領取。全前端 50 suites／173 tests、production build 與 `git diff --check` 均成功；仍待部署測試站後以真實手機確認提示音、播放器拖曳及教材／主題切換畫面。
