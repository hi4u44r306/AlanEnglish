...
- PR #38：學生頭像改為先在瀏覽器裁切成略圓角的正方形，排行榜與個人卡同步使用；手機個人卡改以等級徽章與全寬 XP／AE Points 資訊列消除右側空白。已合併至 `main` commit `93842070` 並完成 Netlify 正式部署；仍待真實學生帳號的桌機、412px 與 iPhone Safari 視覺驗收。
- PR #40：生日欄位在 560px 以下改為垂直排列，避免與儲存按鈕重疊；頭像裁切視窗保留桌面 Pointer 拖移並新增 iPhone Touch 拖移。通知改為受保護的獨立 `/student/notifications` 頁面，可標示已讀與分頁載入較早通知；Header／手機鈴鐺均導向新頁面。`membership-manager` 的 notifications action 仍會從 Firebase 驗證後的呼叫者取得 `student_id`，只新增 1～100 筆的安全分頁與到期通知過濾。PR 已於 2026-08-26 合併至 `main` commit `5518922`，Netlify 正式站已發布同一 commit；`membership-manager` v25 已部署並為 ACTIVE，且維持 `verify_jwt = false` 由函式內 Firebase 驗證。本機 3 份相關 Jest 測試、Production build 與 Deno Edge Function check 均通過；仍待真實學生帳號的 iPhone Safari 與桌機視覺驗收。
