...
- iPhone 實機自測：正式站已有 1 筆啟用中的 Apple Push 訂閱。為避免發布假班級作業，追加一個由學生本人手動按下的裝置自測操作；後端再次確認 Firebase 身分與該裝置的有效訂閱，建立明確標為測試的站內通知及稽核工作，推播僅送此裝置。夜間停發，同分鐘防重複，並計入每日三則上限。功能尚未部署，手機實際收件仍待驗證。
