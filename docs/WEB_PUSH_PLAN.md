...
1. 登入頁先讓學生選擇「登入後設定」或「稍後再說」，預設稍後再說；老師與管理員維持原登入導向。選擇設定的學生在登入完成（首次帳號引導完成）後進入通知頁，仍須親自按「開啟此裝置推播」才要求系統權限。選擇稍後再說不影響登入，日後可在「我的設定」直接開啟或關閉。不能在填寫密碼或頁面載入時自動彈出系統授權。
...
4. 使用者可在通知頁或「我的設定」關閉目前裝置推播；登出時解除此裝置與學生帳號的連結。站內通知與家長 Email 不受影響。其他裝置各自設定。
