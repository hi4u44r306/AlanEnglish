...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        if (action === "cost_dashboard") {
            ...
            const loadOptionalDashboardRows = async (table: string, columns: string, label: string) => {
                const rows: any[] = [];
                ...
                for (let offset = 0; offset < DASHBOARD_MAX_ROWS; offset += DASHBOARD_PAGE_SIZE) {
                    ...
                    if (error) {
                        console.error(`${label} cost dashboard read error`, error);
                        return [];
                    }
                    ...
                    if (currentPage.length < DASHBOARD_PAGE_SIZE) break;
                    if (rows.length >= DASHBOARD_MAX_ROWS) truncated = true;
                }

                return rows;
            };

            const [speakingJobs, speakingChunks, ttsAssets] = await Promise.all([
                loadOptionalDashboardRows(
                    "speaking_generation_jobs",
                    "id,status,model,input_tokens,output_tokens,total_tokens,error_code,created_at",
                    "Speaking generation"
                ),
                loadOptionalDashboardRows(
                    "speaking_source_chunks",
                    "id,status,ocr_model,input_tokens,output_tokens,total_tokens,error_code,created_at",
                    "Speaking OCR"
                ),
                loadOptionalDashboardRows(
                    "speaking_tts_assets",
                    "id,status,voice_id,used_characters,byte_size,error_code,created_at",
                    "Google TTS"
                )
            ]);
            ...
            if (truncated) {
                alerts.push({ level: "critical", code: "row_limit", title: "統計資料已達安全上限", message: `單月紀錄超過 ${DASHBOARD_MAX_ROWS.toLocaleString("en-US")} 筆，畫面可能低估實際費用，請立即核對供應商帳單。` });
            }
