...
declare
    ...
    v_is_confirmed_sandbox_test boolean := false;
begin
    select * into v_actor
    ...
    -- One-time, exact account exception explicitly approved by the project owner.
    -- A following migration restores the normal no-history deletion policy.
    v_is_confirmed_sandbox_test := lower(coalesce(v_target.email, ''))
        = 'ai-e2e-20260824@example.invalid';
    ...
    return jsonb_build_object(
        ...
        'is_confirmed_sandbox_test', v_is_confirmed_sandbox_test,
        'can_delete', v_is_confirmed_sandbox_test
            or coalesce(array_length(v_blockers, 1), 0) = 0,
...
comment on function public.get_student_account_deletion_eligibility(bigint, bigint) is
    'Admin-only preflight with a temporary exact exception for the confirmed Sandbox Test cleanup.';
