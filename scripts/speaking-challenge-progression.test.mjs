...
test("主題練習不阻擋課本順序，完成入門後即可自由開啟", () => {
    ...
    assert.deepEqual(afterPreparation.map(item => [item.id, item.is_unlocked]), [[7, true], [10, true], [11, false], [3, true]]);

    const afterFirstTextbook = speakingChallengeUnlockState(grouped, new Set([701, 1001]));
    assert.deepEqual(afterFirstTextbook.map(item => [item.id, item.is_unlocked]), [[7, true], [10, true], [11, true], [3, true]]);
});
