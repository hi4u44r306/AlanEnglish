...
assert.equal(spokenExampleText("He is [爸爸的年齡] years old."), "He is forty years old.");
assert.equal(spokenExampleText("His name is [爸爸的名字]."), "His name is Alex.");
assert.equal(spokenExampleText("[字母數] letters. [名字拼字]"), "four letters. A L A N");
...
assert.deepEqual(chooseSpeakingVoice(12, 1, voices), { gender: "female", voiceId: "en-US-Chirp3-HD-Leda" });
assert.deepEqual(chooseSpeakingVoice(13, 0, voices), { gender: "female", voiceId: "en-US-Chirp3-HD-Leda" });
assert.deepEqual(chooseSpeakingVoice(13, 1, voices), { gender: "female", voiceId: "en-US-Chirp3-HD-Leda" });
...
assert.match(managerSource, /female: DEFAULT_FEMALE_VOICE_ID/);
